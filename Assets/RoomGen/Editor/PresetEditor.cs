...
namespace RoomGen
{
    ...
    public class PresetEditor : Editor
    {
        ...
        void CreateDecorList(SerializedProperty element, Rect rect, int index)
        {
            ...
            SerializedProperty positionOffset = element.FindPropertyRelative("positionOffset");
            SerializedProperty rotationOffset = element.FindPropertyRelative("rotationOffset");
            SerializedProperty spacingBuffer = element.FindPropertyRelative("spacing");
            SerializedProperty safeAreaBuffer = element.FindPropertyRelative("safeArea");
            SerializedProperty placementHeightRange = element.FindPropertyRelative("verticalRange");
            placementHeightRange.vector2Value = ValidateVector2(placementHeightRange.vector2Value);
            SerializedProperty amountRange = element.FindPropertyRelative("amountRange");
            amountRange.vector2IntValue = ValidateVector2Int(amountRange.vector2IntValue);
            SerializedProperty randomRotation = element.FindPropertyRelative("randomRotation");
            SerializedProperty scaleRange = element.FindPropertyRelative("scaleRange");
            scaleRange.vector2Value = ValidateVector2(scaleRange.vector2Value);
            ...
            if (isExpanded.boolValue)
            {
                ...
                rect.y += lineHeight + lineHeightSpace;
                EditorGUI.PropertyField(new Rect(rect.x + indent, rect.y, rect.width - trailingSpace, lineHeight), safeAreaBuffer);
                rect.y += lineHeight + lineHeightSpace;
        ...
        void AddDecorElement(ReorderableList list)
        {
            ...
            element.FindPropertyRelative("verticalRange").vector2Value = new Vector2(0f, 100f);
            element.FindPropertyRelative("randomRotation").floatValue = 0;
            element.FindPropertyRelative("scaleRange").vector2Value = new Vector2(1f, 1f);
            element.FindPropertyRelative("amountRange").vector2IntValue = new Vector2Int(1, 5);
            element.FindPropertyRelative("isExpanded").boolValue = true;

            list.serializedProperty.serializedObject.ApplyModifiedProperties();
        }

        private Vector2 ValidateVector2(Vector2 input)
        {
            if (input.x > input.y)
            {
                input.y = input.x + 1;
        ...
        private Vector2Int ValidateVector2Int(Vector2Int input)
        {
            if (input.x > input.y)
            {
                input.y = input.x + 1;
